...
int initSuperMarket(SuperMarket* pMarket, const char* fileName,
	const char* customersFileName, const char* fileNameCompressed, int isCompressedFile)
{
	...
	if (isCompressedFile)
	{
		if (loadSuperMarketFromCompressedFile(pMarket, fileNameCompressed, customersFileName) == 1)
		{
			printf("Supermarket successfully loaded from  compressed files\n");
			return 1;
		}
	}
	else if (loadSuperMarketFromFile(pMarket, fileName, customersFileName) == 1)
	{
